...
export default memo(({ server, className }: { server: Server; className?: string }) => {
    ...
    return (
        <Link to={`/server/${server.id}`} className={`block group ${className || ''}`}>
            <div className="relative overflow-hidden rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 p-4 sm:p-5 hover:border-blue-500/40 transition-all duration-300 hover:scale-[1.01] hover:shadow-xl hover:shadow-blue-500/10">
                ...
                <div className="relative flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                <h3 className="text-base sm:text-lg font-semibold text-white truncate">{server.name}</h3>
                                {server.description && (
                                    <p className="text-xs sm:text-sm text-blue-200/60 truncate">{server.description}</p>
                                )}
                            </div>
                        </div>
                        
                        <div className="flex items-center gap-2 text-xs sm:text-sm text-blue-200/40 mt-1 ml-0 sm:ml-13">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        ...
                        {!isSuspended && !server.isNodeUnderMaintenance && stats && (
                            <>
                                ...
                                <div className="flex items-center gap-1 sm:gap-2">
                                    <Cpu className={`w-3 h-3 sm:w-4 sm:h-4 ${cpuPercent >= 90 ? 'text-red-400' : 'text-blue-400/60'}`} />
                                    ...
                                    <span className={`text-[10px] sm:text-xs ${cpuPercent >= 90 ? 'text-red-400' : 'text-blue-200/60'}`}>
                                ...
                                {/* Memory */}
                                <div className="flex items-center gap-1 sm:gap-2">
                                    <MemoryStick className={`w-3 h-3 sm:w-4 sm:h-4 ${isAlarm(stats.memoryUsageInBytes, server.limits.memory) ? 'text-red-400' : 'text-blue-400/60'}`} />
                                    <div className="w-12 sm:w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                        <div className={`h-full rounded-full transition-all duration-500 ${
                                            isAlarm(stats.memoryUsageInBytes, server.limits.memory) ? 'bg-red-500' : 'bg-gradient-to-r from-purple-500 to-purple-400'
                                        }`} style={{ width: `${Math.min(memoryPercent, 100)}%` }} />
                                    </div>
                                    <span className={`text-[10px] sm:text-xs ${isAlarm(stats.memoryUsageInBytes, server.limits.memory) ? 'text-red-400' : 'text-blue-200/60'}`}>
                                ...
                                {/* Disk */}
                                <div className="flex items-center gap-1 sm:gap-2">
                                    <HardDrive className={`w-3 h-3 sm:w-4 sm:h-4 ${isAlarm(stats.diskUsageInBytes, server.limits.disk) ? 'text-red-400' : 'text-blue-400/60'}`} />
                                    <div className="w-12 sm:w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                        <div className={`h-full rounded-full transition-all duration-500 ${
                                            isAlarm(stats.diskUsageInBytes, server.limits.disk) ? 'bg-red-500' : 'bg-gradient-to-r from-orange-500 to-orange-400'
                                        }`} style={{ width: `${Math.min(diskPercent, 100)}%` }} />
                                    </div>
                                    <span className={`text-[10px] sm:text-xs ${isAlarm(stats.diskUsageInBytes, server.limits.disk) ? 'text-red-400' : 'text-blue-200/60'}`}>
                                        {bytesToString(stats.diskUsageInBytes)}
                                    </span>
                                </div>

                                <div className="flex items-center gap-1 text-blue-200/40 text-[10px] sm:text-xs">
                        ...
                        <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 text-blue-400/40 group-hover:text-blue-400 group-hover:translate-x-1 transition-all duration-300" />
                    </div>
                </div>
            </div>
        </Link>
